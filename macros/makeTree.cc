...
//if(!isSimulation) loop.readSectorFileList("/lustre/nyx/cbm/users/ogolosov/HADES/treeMaker/src/list_gen109_day108.txt",kFALSE,kFALSE);
    ...
    const Short_t nModules = 304;              //number of modules in FW
    ...
    const Short_t maxNWallModule=304;
   ...
   /* tree->Branch("cuts",            &cuts,            "cuts[nCuts]/O");
    ...
    tree->Branch("wallChargeTot_module", &wallChargeTot_module,   "wallChargeTot_module[maxNWallModule]/F");
*/    
...
tree->Branch("cuts",             cuts,            TString::Format("cuts[%i]/O", nCuts));
    ...
    tree->Branch("wallHitTime",         wallHitTime,            TString::Format("wallHitTime[%i]/F", nWallHitsTot));  
    tree->Branch("wallHitCharge",       wallHitCharge,          TString::Format("wallHitCharge[%i]/F", nWallHitsTot));
    tree->Branch("wallHitDistance",     wallHitDistance,        TString::Format("wallHitDistance[%i]/F", nWallHitsTot));
    tree->Branch("wallHitRing",         wallHitRing,            TString::Format("wallHitRing[%i]/S", nWallHitsTot));
    tree->Branch("wallHitPhi",          wallHitPhi,             TString::Format("wallHitPhi[%i]/F", nWallHitsTot));
    ...
    tree->Branch("wallChargeTot_mod", &wallChargeTot_mod,  TString::Format("wallChargeTot_module[%i]/F", nModules));
...
//#######################################################################
	...
	for (Short_t l=1; l<=nModules; l++) {
	wallChargeTot_mod[l-1]=0;	
	}
	    ...
	    for(Short_t j=0; j<nWallHitsTot; j++) {
            ...
            if ( (ring<=4            && wallHitCharge[j]>80 && hit_beta>0.84 && hit_beta<1.) ||
                ...
                for (Short_t i=0; i < nRings; i++){
                    if (ring == i) wallChargeTot_ring[i]+=wallHitCharge[j];
                }

		for (Short_t k=1; k<=nModules; k++) {
