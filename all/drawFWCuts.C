...
//tree1->Add("/lustre/nyx/hades/user/ezherebt/treeMaker/output/Nov_23_14_46/tree_1210816*.root");
//Nov_10_11_48/tree_1210816
tree1->Add("/lustre/nyx/hades/user/tmamont/gitDir/HADES/treeMaker/output/Nov_24_16_06/AuAu_1_23AGev_gen8_108.list/tree_1210816*.root");
	...
	TH1F *hFWphi=new TH1F("hFWphi","",628,0,6.28);
	...
	int entry=0;
		...
		if (cuts[0] && cuts[1] && cuts[2] && cuts[3] && cuts[4] && cuts[5] && cuts[6] && cuts[7]) {
			...
			for (short i=0;i<nWallHitsTot;i++) {

			FWphi=wallHitPhi[i];
			FWdist=wallHitDistance[i];
			FWtime=wallHitTime[i];
...
TCanvas *c006 = new TCanvas("c006");
        c006->cd();
        gPad-> SetLogy();
	hFWphi->Draw();

TCanvas *c005 = new TCanvas("c005");
        c005->cd();
        gPad-> SetLogy();
	hFWtime->Draw();

TCanvas *c004 = new TCanvas("c004");
        c004->cd();
        gPad-> SetLogy();
	hFWdist->Draw();

TCanvas *c003 = new TCanvas("c003");
        c003->cd();
        gPad-> SetLogy();
	...
	TLegend *leg003 = new TLegend(0.2, 0.15, 0.6, 0.4);
	leg003->SetFillColor(0);
        leg003->SetBorderSize(0);
        leg003->SetTextSize(0.03);
        leg003->SetTextFont(42);
	

	sprintf(tr, "%s", name3);
	leg003->AddEntry(FW_All, tr, "l");
	sprintf(tr, "%s", name4);
        leg003->AddEntry(FW_Cut1, tr, "l");
        sprintf(tr, "%s", name5);
        leg003->AddEntry(FW_Cut2, tr, "l");
        sprintf(tr, "%s", name6);
        leg003->AddEntry(FW_Cut3, tr, "l");
        sprintf(tr, "%s", name7);
        leg003->AddEntry(FW_Cut4, tr, "l");
        sprintf(tr, "%s", name8);
        leg003->AddEntry(FW_Cut5, tr, "l");
        sprintf(tr, "%s", name9);
        leg003->AddEntry(FW_Cut6, tr, "l");
        sprintf(tr, "%s", name10);
        leg003->AddEntry(FW_Cut7, tr, "l");
        sprintf(tr, "%s", name11);
        leg003->AddEntry(FW_Cut8, tr, "l");

        leg003->Draw();
        c003->Print("./pics/FWch_DiffCutsAtt.png");
        c003->SaveAs("./pics/FWch_DiffCutsAtt.C","RECREATE");
